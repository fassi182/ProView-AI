...
# Import your AI logic modules directly
...
<script>
// Auto-cleanup when tab is closed or user navigates away
window.addEventListener('beforeunload', function(e) {
    // Mark session for cleanup
    sessionStorage.setItem('proview_cleanup', 'true');
});

// Check if we should cleanup on page load
window.addEventListener('load', function() {
    if (sessionStorage.getItem('proview_cleanup') === 'true') {
        sessionStorage.removeItem('proview_cleanup');
        // Session was closed, data should be cleaned
    }
});
</script>
...
# Check for session timeout (auto-cleanup after inactivity)
...
if time.time() - st.session_state.last_activity > SESSION_TIMEOUT_SECONDS:
    # Session timed out - clean up and restart
    try:
...
def initialize_system():
    """Initialize the ProView system"""
    try:
        ProViewConfig.validate()
        
        # Run background cleanup to remove old sessions (silently)
        try:
            janitor_cleanup()
...
def process_uploaded_file(uploaded_file, session_id: str) -> tuple[bool, str]:
    ...
    try:
        ...
        try:
            ...
            # Process file using your RAG storage
...
def reset_session():
    ...
    try:
        # Clear server-side data using your storage module
        clear_session_data(st.session_state.session_id)
...
def get_ai_response(user_message: str, history: list, session_id: str) -> dict:
    """
    Get AI response directly from your services module
    ...
    """
    try:
        # Convert Streamlit message format to your format
        formatted_history = []
        ...
        # Get response using your service
...
with st.sidebar:
    ...
    with st.expander("ℹ️ What's the difference?", expanded=False):
        ...
        **Run Cleanup Now (below):**
        - ⚠️ Removes OLD data from ALL users (>2h old)
        - ❌ Does NOT affect YOUR current session
        - Use: Admin maintenance task
        """)
    ...
    with col_clear2:
        if st.button("Clear All Data", use_container_width=True, type="primary", help="Clears chat + uploaded documents"):
    ...
    with st.expander("🔒 Privacy & Data", expanded=False):
        ...
        st.caption("✅ Each session is isolated (others can't see your data)")
...
for msg in st.session_state.messages:
    with st.chat_message(msg["role"]):
        if isinstance(msg["content"], dict):
            ...
            # Feedback section (if answer was evaluated)
...
if prompt := st.chat_input("Start by telling ProView which role you're interviewing for..."):
    ...
    with st.chat_message("assistant"):
        ...
        # Get AI response directly from your services
...
st.caption("💡 **Deployed on Streamlit Cloud?** Your vector DB is temporary and never committed to GitHub.")